...
export default function IncomesPage() {
  ...
  function load() {
    incomeService.list().then((p) => setItems(p.content))
  }
  ...
  const columns = [
    ...
    { key: 'amount', label: 'Valor', primary: true, render: (i) => <span className="font-semibold text-emerald-600">{formatCurrency(i.amount)}</span> },
    { key: 'date', label: 'Fecha', render: (i) => formatDate(i.incomeDate) },
  ...
  return (
    <div className="space-y-4">
        ...
        <form onSubmit={handleCreate} className="bg-white border border-slate-200 rounded-lg p-4 space-y-3 max-w-md">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label>
              ...
              <input required type="date" value={form.incomeDate} onChange={(e) => setForm({ ...form, incomeDate: e.target.value })} className={inputClass} />
            </label>
          </div>
          <label className="block">
            <span className="block text-xs font-medium text-slate-600 mb-1">Categoría</span>
            <select value={form.incomeCategoryId} onChange={(e) => setForm({ ...form, incomeCategoryId: e.target.value })} className={inputClass}>
      ...
      <ResponsiveList columns={columns} rows={items} keyExtractor={(i) => i.id} emptyMessage="No hay ingresos manuales registrados." />
    </div>
  )
}
